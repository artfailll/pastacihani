...
(function () {
  ...
  if (galGrid) {
    ...
    Promise.all(GAL_CATS.map(slug =>
      fetch(`https://res.cloudinary.com/${CLOUD}/image/list/${slug}.json?ts=`+Date.now())
        .then(r=>r.ok?r.json():{resources:[]})
        .then(d=>({ slug, r:(d.resources||[]).sort((a,b)=>(b.created_at||"").localeCompare(a.created_at||""))[0]||null }))
        .catch(()=>({ slug, r:null }))
    )).then(results=>{
      galGrid.innerHTML="";
      const lbItems=[];
      results.forEach(({slug,r})=>{
        if(!r)return;
        const span=SPANS[GAL_CATS.indexOf(slug)]||"";
        const w=span==="wide"?900:560, h=span==="tall"?1100:span==="wide"?560:720;
        const src=cldGal(r,w,h);
        lbItems.push({img:`https://res.cloudinary.com/${CLOUD}/image/upload/f_auto,q_auto,w_1300/v${r.version}/${r.public_id}.${r.format}`, title:GAL_LABELS[slug]});
        const item=document.createElement("div");
        item.className="gal-item reveal"+(span?" "+span:"");
        item.innerHTML=`<img src="${src}" alt="${GAL_LABELS[slug]} pasta tasarımı" loading="lazy"/><div class="gov"><span class="gt">${GAL_LABELS[slug]}</span></div>`;
        const idx=lbItems.length-1;
        item.addEventListener("click",()=>openLightbox(lbItems,idx));
  ...
  [...TESTIMONIALS, ...TESTIMONIALS].forEach((t) => track.appendChild(buildT(t)));
